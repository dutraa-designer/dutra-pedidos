...
export async function DELETE(request: Request) {
  try { await prepare(); const body = await request.json() as { id?: number }; const id = Number(body.id); if (!id) return Response.json({ error: "Registro inválido." }, { status: 400 }); const existing = await getDb().execute(sql`SELECT amount FROM payments WHERE id = ${id}`); if (!existing.rows[0]) return Response.json({ error: "Pagamento não encontrado." }, { status: 404 }); await getDb().execute(sql`DELETE FROM payments WHERE id = ${id}`); return Response.json({ ok: true, amount: Number(existing.rows[0].amount) }); }
  catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Não foi possível excluir o pagamento." }, { status: 400 }); }
}

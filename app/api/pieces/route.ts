...
async function ensureMaterialColumns() { await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS gold_gram_value real`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS gold_factor real`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS custom_material text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mail_posted_confirmed integer NOT NULL DEFAULT 0`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mail_posted_at text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mandabem_ref_id text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mandabem_envio_id text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mandabem_label text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mandabem_status text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mandabem_checked_at text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS mandabem_delivered_at text`); await getDb().execute(sql`ALTER TABLE pieces ADD COLUMN IF NOT EXISTS sort_order integer`); }
...
function validatePayload(payload: Record<string, unknown>, partial = false) {
  ...
  return {
    ...
    ...(payload.mandabemRefId !== undefined ? { mandabemRefId: cleanText(payload.mandabemRefId) } : {}),
    ...(payload.mandabemEnvioId !== undefined ? { mandabemEnvioId: cleanText(payload.mandabemEnvioId) } : {}),

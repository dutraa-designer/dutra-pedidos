...
export default function Home() {
  ...
  const focusFilter = (nextFilter: string) => { setQuery(""); setFilter(nextFilter); document.querySelector(".workspace-card")?.scrollIntoView({ behavior: "smooth", block: "start" }); };
  ...
  return <main className="app-shell"><div className="app-orb orb-one" /><div className="app-orb orb-two" /><div className="app-container">
    ...
    <section className="stats-grid" aria-label="Resumo da produção"><button className="stat-card stat-card-main stat-button" onClick={() => focusFilter("TODOS")}><div className="stat-icon violet"><Package size={19} /></div><div><span className="stat-label">Peças no controle</span><strong>{stats.total}</strong><small>ver todos os cadastros</small></div></button><button className="stat-card stat-button" onClick={() => focusFilter("PRODUÇÃO")}><div className="stat-icon amber"><RefreshCw size={19} /></div><div><span className="stat-label">Em produção</span><strong>{stats.production}</strong><small>acompanhar agora</small></div></button><button className="stat-card stat-button" onClick={() => focusFilter("BANHO")}><div className="stat-icon blue"><WandSparkles size={19} /></div><div><span className="stat-label">Aguardando banho</span><strong>{stats.bath}</strong><small>prontas para encaminhar</small></div></button><button className="stat-card stat-button" onClick={() => focusFilter("ENVIO")}><div className="stat-icon coral"><Send size={19} /></div><div><span className="stat-label">Envios pendentes</span><strong>{stats.shipping}</strong><small>resolver pendências</small></div></button><button className="stat-card cost-card stat-button" onClick={() => focusFilter("PAGAMENTO")}><div className="cost-metric"><span className="stat-label">Custos lançados</span><strong>{money.format(stats.cost)}</strong><small>produção + banho + transporte</small></div><div className="cost-divider" aria-hidden="true" /><div className="cost-metric cost-open"><span className="stat-label">Falta pagar</span><strong>{money.format(stats.open)}</strong><small>ver pagamentos em aberto</small></div></button></section>

...
import { Check, CircleHelp, Edit3, Package, Plus, RefreshCw, Search, Send, Trash2, WandSparkles, X } from "lucide-react";
...
type Piece = {
  ...
  mailStatus: string | null; billingStatus: string | null; notes: string | null;
};
...
const emptyForm: FormState = { code: 10, description: "", quantity: 1, productionStatus: "EM PRODUÇÃO", productionValue: 35, bathStatus: null, bathValue: null, shippingStatus: null, transportValue: null, mailStatus: null, billingStatus: null, notes: null };
...
export default function Home() {
  ...
  const stats = useMemo(() => ({
    ...
    shipping: pieces.filter((p) => p.mailStatus !== "ENVIADO").length, cost: pieces.reduce((sum, p) => sum + totalCost(p), 0),
  }), [pieces]);
  const visiblePieces = useMemo(() => pieces.filter((piece) => {
    ...
    const matchesFilter = filter === "TODOS" || (filter === "PRODUÇÃO" && piece.productionStatus !== "FINALIZADO") || (filter === "BANHO" && piece.productionStatus === "FINALIZADO" && piece.bathStatus !== "FINALIZADO") || (filter === "ENVIO" && piece.mailStatus !== "ENVIADO") || (filter === "FATURAMENTO" && piece.billingStatus !== "OK");
